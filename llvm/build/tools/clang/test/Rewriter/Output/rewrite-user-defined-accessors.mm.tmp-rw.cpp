...
static Foo * _I_Foo_foo(Foo * self, SEL _cmd) {
    if (!((Foo_IMPL *)self)->foo) {
        ((Foo_IMPL *)self)->foo = 0;
    }
    return ((Foo_IMPL *)self)->foo;
}



static void _I_Foo_setFoo_(Foo * self, SEL _cmd, Foo *arg) {
  ((Foo_IMPL *)self)->foo = arg;
}
...
static struct {
	...
	1
	,{{"foo", "@\"Foo\"", __OFFSETOFIVAR__(struct Foo_IMPL, foo)}
	 }
};
...
static struct {
	...
	0, 2
	,{{(SEL)"foo", "@16@0:8", (void *)_I_Foo_foo}
	  ,{(SEL)"setFoo:", "v24@0:8@16", (void *)_I_Foo_setFoo_}
	 }
};
